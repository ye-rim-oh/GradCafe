...
# 01_scrape_all_years.R -- Scrape 2020-2026 from GradCafe (Unified)
# Single script that scrapes ALL years with the same parser
...
# Season codes: F20 = Fall 2020, ..., F26 = Fall 2026
...
normalize_institution <- function(school) {
  case_when(
    ...
    grepl("Toronto", school, ignore.case = TRUE) ~ "Toronto",
    grepl("McGill", school, ignore.case = TRUE) ~ "McGill",
...
# MAIN EXECUTION
# =====================================================================
cat("=== Starting Unified GradCafe Scraper (2020-2026) ===\n")
...
for (yr in target_years) {
  ...
  if (length(year_results) > 0) {
    ...
    # Add institution normalization
    year_clean <- year_clean %>%
      mutate(institution = normalize_institution(school))
